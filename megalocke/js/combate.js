...
function renderPokemonCard(pokemonData) {
    ...
    // **IMPORTANTE**: Aquí no se añade la insignia de doble, ya que es para el entrenador.
...
async function renderContent(searchTerm = '') {
    ...
    try {
        ...
        filteredProfiles.forEach((profileData, profileIndex) => {
            ...
            // **NUEVO CÓDIGO para la insignia del entrenador: Se define si el combate es doble**
            const trainerDoubleBadge = profileData.trainer.esDoble ? '<span class="badge trainer-double-badge">🤺 2 vs 2</span>' : '';
